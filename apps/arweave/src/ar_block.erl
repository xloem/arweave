...
-export([is_2_6_repacking_complete/1, block_field_size_limit/1, verify_timestamp/2,
		...
		get_packing_threshold/2, get_packing_2_6_threshold/2, shift_packing_2_6_threshold/1]).
...
verify_weave_size(NewB, OldB, TXs) ->
	...
	BlockSize == NewB#block.block_size
...
%% two recall subspaces on the weave as unlocked by the given nonce limiter output.
compute_h0(NonceLimiterOutput, SearchSpaceNumber, PrevSolutionHash, MiningAddr) ->
	crypto:hash(sha256, << NonceLimiterOutput:32/binary, SearchSpaceNumber:256,
			PrevSolutionHash:32/binary, MiningAddr:33/binary >>).
...
compute_h1(H0, Nonce, Chunk) ->
	...
	{crypto:hash(sha256, << H0:32/binary, Preimage/binary >>), Preimage}.
...
%% @doc Return {RecallSubspace1Start, RecallSubspace2Start} - the start offsets
%% of the two recall subspaces.
get_recall_space(H0, SearchSpaceNumber, SearchSpaceUpperBound) ->
	RecallSubspace1Start = SearchSpaceNumber * ?SEARCH_SPACE_SIZE
			+ binary:decode_unsigned(binary:part(H0, 0, 8), big) rem ?SEARCH_SPACE_SIZE,
	RecallSubspace2Start = binary:decode_unsigned(H0, big) rem SearchSpaceUpperBound,
	{RecallSubspace1Start, RecallSubspace2Start}.
...
get_packing_2_6_threshold(B, SearchSpaceUpperBound) ->
