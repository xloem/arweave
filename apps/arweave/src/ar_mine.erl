...
validate_spora(Args) ->
	{BDS, Nonce, Timestamp, Height, Diff, PrevH, SearchSpaceUpperBound, Packing_2_6_Threshold,
			...
			Thread = spawn(
				fun() ->
					small_weave_hashing_thread({Timestamp, Diff, BDS, PrevH,
							SearchSpaceUpperBound, StageTwoHasher, Parent, Height, SessionRef})
				end
			),
		...
		{solution, Nonce, H0, Timestamp, Hash} ->
			case maps:get(Timestamp, BlocksByTimestamp, not_found) of
				...
				{#block{ timestamp = Timestamp } = B, BDS} ->
					...
					case get_spoa(H0, PrevH, SearchSpaceUpperBound, Height, PackingThreshold,
	...
	receive
		{event, chunk_storage, {removed_file, Key}} ->
			?LOG_DEBUG([{event, closing_handle_of_removed_chunk_storage_file},
					{key, Key}]),
			ar_chunk_storage:close_file(Key),
			io_thread(SearchInRocksDB);
...
io_thread_read_chunk(Args) ->
	{EncodedByte, H0, Nonce, HashingThread, Entropy, Timestamp, Diff, SessionRef,
			PackingThreshold, SearchInRocksDB, Height, Packing_2_6_Threshold,
			RewardAddr} = Args,
	Byte = binary:decode_unsigned(EncodedByte, big),
	case read_chunk(Byte, PackingThreshold, SearchInRocksDB, Height, Packing_2_6_Threshold,
			RewardAddr) of
		{error, _} ->
			io_thread(SearchInRocksDB);
		{ok, #{ chunk := Chunk }} ->
...
read_chunk(Byte, PackingThreshold, SearchInRocksDB, Height, Packing_2_6_Threshold, RewardAddr) ->
...
get_spoa(H0, PrevH, SearchSpaceUpperBound, Height, PackingThreshold, Packing_2_6_Threshold,
		RewardAddr) ->
	case pick_recall_byte(H0, PrevH, SearchSpaceUpperBound) of
		{error, weave_size_too_small} ->
			{#poa{}, undefined};
...
test_basic() ->
	...
	start({B1, [], unclaimed, [], [], #{}, ar_mine:get_search_space_upper_bound(BI), Threads}),
...
test_timestamp_refresh() ->
	...
	Run = fun(_) ->
		...
		ar_events:subscribe(block),
		start({B, TXs, unclaimed, [], [], #{}, B0#block.weave_size, Threads}),
...
test_start_stop() ->
	...
	PID = start({B#block{ diff = HighDiff }, [], unclaimed, [], [], #{},
			ar_mine:get_search_space_upper_bound(BI), Threads}),
	timer:sleep(500),
	assert_alive(PID),
	stop(PID),
	...
	receive
		{event, block, {mined, NewB, MinedTXs, BH}} ->
			...
			?assertEqual(spora_solution_hash(PrevH, Timestamp, H0, Chunk, Height),
					{NewB#block.hash, NewB#block.hash_preimage}),
