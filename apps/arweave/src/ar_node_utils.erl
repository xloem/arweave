...
validate(NewB, B, Wallets, BlockAnchors, RecentTXMap, SearchSpaceUpperBound) ->
	?LOG_INFO([{event, validating_block}, {hash, ar_util:encode(NewB#block.indep_hash)}]),
	case timer:tc(
		fun() ->
			do_validate(NewB, B, Wallets, BlockAnchors, RecentTXMap, SearchSpaceUpperBound)
		end
	) of
		{TimeTaken, valid} ->
...
do_validate(NewB, OldB, Wallets, BlockAnchors, RecentTXMap, SearchSpaceUpperBound) ->
...
validate_block(weave_size, {#block{ txs = TXs } = NewB, OldB, Wallets, BlockAnchors,
		RecentTXMap, SearchSpaceUpperBound}) ->
	...
	case OldB#block.indep_hash == NewB#block.previous_block of
		...
		true ->
			validate_block(previous_solution_hash,
				{NewB, OldB, Wallets, BlockAnchors, RecentTXMap, SearchSpaceUpperBound})
	end;
validate_block(previous_solution_hash, {NewB, OldB, Wallets, BlockAnchors, RecentTXMap,
		SearchSpaceUpperBound}) ->
	case ar_block:is_2_6_repacking_complete(NewB) of
		true ->
...
validate_block(packing_2_6_threshold, {NewB, OldB, Wallets, BlockAnchors, RecentTXMap,
		SearchSpaceUpperBound}) ->
	ExpectedPackingThreshold = ar_block:get_packing_2_6_threshold(OldB, SearchSpaceUpperBound),
	Valid =
		case ExpectedPackingThreshold of
			undefined ->
				true;
...
validate_block(spora, {NewB, OldB, Wallets, BlockAnchors, RecentTXMap,
	...
	case ar_mine:validate_spora({BDS, Nonce, Timestamp, Height, Diff, PrevH,
			SearchSpaceUpperBound, Packing_2_6_Threshold, StrictDataSplitThreshold, RewardAddr,
			POA}) of
		{false, Hash} ->
...
test_block_validation() ->
	...
	?assertEqual({invalid, invalid_weave_size},
			validate(B#block{ weave_size = PrevB#block.weave_size + 1 }, PrevB, Wallets,
					BlockAnchors, RecentTXMap, SearchSpaceUpperBound)),
	...
	?assertEqual({invalid, invalid_spora_hash},
			validate(InvDataRootB#block{ indep_hash = ar_block:indep_hash(InvDataRootB) },
					PrevB, Wallets, BlockAnchors, RecentTXMap, SearchSpaceUpperBound)),
	...
	?assertEqual({invalid, invalid_spora_hash},
			validate(
				InvLastRetargetB#block{ indep_hash = ar_block:indep_hash(InvLastRetargetB) },
				PrevB, Wallets, BlockAnchors, RecentTXMap, SearchSpaceUpperBound)),
	?assertEqual({invalid, invalid_difficulty},
			validate_block(difficulty, {
	...
	?assertEqual({invalid, invalid_spora_hash},
			validate(B2#block{ hash = B#block.hash }, B, Wallets, BlockAnchors2,
					RecentTXMap2, SearchSpaceUpperBound2)),
	PoA = B2#block.poa,
	FakePoA =
		case get_chunk(1) of
	...
	{FakeH0, FakeEntropy} = ar_mine:spora_h0_with_entropy(
			...
			B2#block.height),
	{FakeSolutionHash, _} = ar_mine:spora_solution_hash_with_entropy(B#block.indep_hash,
	...
	?assertEqual({invalid, invalid_spora},
			validate(B2#block{ poa = FakePoA, hash = FakeSolutionHash }, B, Wallets,
					BlockAnchors2, RecentTXMap2, SearchSpaceUpperBound2)),
	?assertEqual({invalid, invalid_spora_hash},
			validate(B2#block{ poa = FakePoA, hash = FakeSolutionHashNoEntropy }, B,
					Wallets, BlockAnchors2, RecentTXMap2, SearchSpaceUpperBound2)).
