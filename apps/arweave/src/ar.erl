	...
	lists:foreach(
		...
		[
			...
			{"mining_addr (addr)",
				...
				" created. Set 'unclaimed' to send all the rewards to the endowment pool.",
				[?WALLET_DIR])},
			{"mining_addr_2_6 (addr)",
				io_lib:format(
				"The address mining rewards should be credited to after the fork 2.6. Note"
				" that for some time after the fork 2.6, it makes sense to specify both"
				" mining_addr and mining_addr_2_6 because rewards will be claimed differently"
				" based on the chosen recall chunks. mining_addr_2_6 must be an address of"
				" either an ECDSA or an EDDSA wallet. A wallet with this address must be"
				" present in the [data_dir]/~s directory. If the directory already"
				" contains such keys, the one written later is picked, no new files are"
				" created. The specified address is also a packing key, so it is used to pack"
				" synced data even if the \"mine\" flag is not specified. If any, the data"
				" already packed with different addresses is not repacked.",
				[?WALLET_DIR])},
...
parse_cli_args(["mining_addr", "unclaimed" | Rest], C) ->
	case C#config.mining_addr of
		...
		_ ->
			io:format("You may specify at most one mining_addr.~n"),
			erlang:halt()
	end;
...
parse_cli_args(["mining_addr_2_6", Addr | Rest], C) ->
	case C#config.mining_addr_2_6 of
		not_set ->
			case ar_util:safe_decode(Addr) of
				{ok, DecodedAddr} when byte_size(DecodedAddr) == 33 ->
					parse_cli_args(Rest, C#config{ mining_addr_2_6 = DecodedAddr });
				_ ->
					io:format("~nmining_addr_2_6 must be a valid Base64Url string, 44"
							" characters long.~n~n"),
					erlang:halt()
			end;
		_ ->
			io:format("~nThere is no support for several mining_addr_2_6 yet.~n~n"),
			erlang:halt()
	end;
...
set_mining_address(#config{ mining_addr = unclaimed } = C) ->
	set_mining_address_2_6(C);
set_mining_address(#config{ mining_addr = not_set, mine = true } = C) ->
	...
	set_mining_address_2_6(C#config{ mining_addr = Addr });
set_mining_address(#config{ mining_addr = not_set } = C) ->
	set_mining_address_2_6(C#config{ mining_addr = unclaimed });
set_mining_address(C) ->
	set_mining_address_2_6(C).

set_mining_address_2_6(#config{ mining_addr_2_6 = not_set } = C) ->
	Key = ar_wallet:get_or_create_wallet([{ecdsa, secp256k1}, {eddsa, ed25519}]),
	Addr = ar_wallet:to_address(Key),
	ar:console("Setting the mining address to ~s.~n", [ar_util:encode(Addr)]),
	application:set_env(arweave, config, C#config{
			mining_addr_2_6 = ar_wallet:to_address(Key) });
set_mining_address_2_6(#config{ mining_addr_2_6 = Addr } = C) ->
	case ar_wallet:load_key(Addr) of
		not_found ->
			ar:console("The mining key for the address ~s was not found."
				" Make sure you placed the file in [data_dir]/~s."
				" Do not specify mining_addr_2_6 if you want one to be generated.",
				[Addr, ?WALLET_DIR]),
			erlang:halt();
		Key ->
			application:set_env(arweave, config, C#config{
